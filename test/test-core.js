...
function testGetQueryVars() {
    QUnit.test("Get http query variables", function(assert) {
        assert.expect(1);
        var query1 = "?foo=b%40r&bob=b";
        var queryVars = UstadJS.getURLQueryVars(query1);
        assert.equal(queryVars.foo, "b@r");
    });
}
